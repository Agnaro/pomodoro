...
import {
  PartialTimerState,
  TimerState,
} from "../services/settings/settings.interface";
...
export class HomePageComponent implements OnInit, OnDestroy {
  ...
  private timerCommands$ = merge(
    ...
    this.timerUI.btnReset$.pipe(
      mapTo({
        time: this.initialTimerState.time,
        isTicking: false,
        onBreak: false,
      })
    ),
    this.timerUI.btnBreak$.pipe(
      mapTo({
        time: this.settingsService.breakTime,
        isTicking: true,
        onBreak: true,
      })
    ),
  ...
  private timerUpdateTrigger$ = combineLatest([
    this.isTicking$,
    this.tickSpeed$,
  ]).pipe(
  ...
  // = SIDE EFFECTS =========================================================
  // == UI INPUTS ===========================================================
  private renderTimeChange$ = this.time$.pipe(
    tap((n) => this.timerUI.renderTimeChange(n))
  );
  private renderOnBreak$ = this.onBreak$.pipe(
    tap((b) => this.timerUI.renderOnBreak(b))
  );
  ...
  // == CREATION METHODS ====================================================
  // == OPERATORS ===========================================================
  private queryChange<T extends Object, K1 extends keyof T>(
    key: K1
  ): UnaryFunction<Observable<T>, Observable<T[K1]>> {
    return pipe(pluck<T, K1>(key), distinctUntilChanged());
  }

  constructor(private settingsService: SettingsService) {}

  ngOnInit() {
    this.subscribeToThis$.pipe(takeUntil(this.destroy$)).subscribe();
  }
